...
class EditTable extends Component {
    ...
    renderInput = (index, name, text, require, props = {}, row) => {
        return <div className={require === true ? (text === '' ? 'has-error' : null) : null}>
            <Input
                {...props}
                value={text}
                className='has-error'
    ...
    renderSelect = (index, name, text, options, require, props = {}, optionConfig) => {
        ...
        return <div className={require === true ? ((text === '' || text === undefined) ? 'has-error' : null) : null}>
            <Select
                {...props}
                value={text}
                style={{ width: '100%' }}
            ...
            >
                {
                    options.map(item => <Option disabled={item.disabled} value={`${item.value}`} key={`${item.value}`}>{item.label}</Option>)
                }
            </Select>
        </div >
    }
    ...
    render() {
        ...
        const state_columns = columns.map(item => {
            const { dataIndex, require, props, optionConfig, type, selectOption } = item
            switch (type) {
                case 'select':
                    item.render = (text, row, index) => this.renderSelect(index, dataIndex, text, selectOption, require, props, optionConfig)
                    break;
                case 'num':
                    item.render = (text, row, index) => this.renderNumberInput(index, dataIndex, text, require, props)
                    break;
                case 'checkbox':
        ...
        return (
            <div>
                {add === false ? null : <Button onClick={this.handleAdd} style={{ marginBottom: 4 }}>添加</Button>}
                <Table
                    title={title}
                    pagination={false}
                    className='table'
